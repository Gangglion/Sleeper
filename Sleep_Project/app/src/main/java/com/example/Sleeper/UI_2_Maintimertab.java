...
//        }
        question.setText(temp + "\n정답은 ??"+"("+answer+")"); //문제 TextView에 설정 + 현재 정답까지 써있는 상태
        ...
        //설정완료 버튼 누르면 기능 동작과 함께 스레드 또한 실행됨 - 화면전환도 관련되어있음
        setBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                String sleepTime="";
                String alarmTime="";
                if(sleep_timePicker.getMinute()<10)
                {
                    if(sleep_timePicker.getHour()<10)
                    {
                        sleepTime="0"+sleep_timePicker.getHour()+":";
                    }
                    sleepTime+="0"+sleep_timePicker.getMinute();
                }
                else
                {
                    sleepTime=sleep_timePicker.getHour()+":"+sleep_timePicker.getMinute();
                }
                if(alarm_timePicker.getMinute()<10)
                {
                    if(alarm_timePicker.getHour()<10)
                    {
                        alarmTime="0"+alarm_timePicker.getHour()+":";
                    }
                    alarmTime+="0"+alarm_timePicker.getMinute();
                }
                else
                {
                    alarmTime=alarm_timePicker.getHour()+":"+alarm_timePicker.getMinute();
                }
                Log.d("checkDBTime", sleepTime);
                Log.d("checkDBTime", alarmTime);
                //로그인 된 정보를 갖고있는 객체
                AboutLogin aboutLogin = new AboutLogin();
                //DB에 시간정보 저장(email, 현재 날짜, 수면예약시간, 취침예약시간)
                try{
                    long nowt = System.currentTimeMillis();
                    String nowDate = (String) DateFormat.format("yyyy-MM-dd", nowt);
                    //입력한 시간이 데이터베이스에 등록됨
                    String result = new DbTask("write_settime").execute("write_settime",
                            aboutLogin.getUser().getEmail(),sleepTime,alarmTime,nowDate).get();
                }catch(Exception e)
                {
                    e.printStackTrace();
                }
